...
self.addEventListener('push', (event) => {
  if (!event.data) {
    console.log('[Service Worker] Push event received but has no data.');
    return;
  }
  ...
  try {
    ...
    const title = payload.title || 'تنبيه من نظام الحضور';
    const options = {
      body: payload.body || '',
    ...
    console.log('[Service Worker] Push event data parse failed, showing text body:', err);
    ...
    event.waitUntil(
      self.registration.showNotification('تنبيه من نظام الحضور', options)
    );
  }
});
