        ...
        if (action === "login") {
            ...
            return res.status(200).json({
                ...
                data: {
                    ...
                    preferredNotificationChannel: user.preferred_notification_channel || 'both'
                }
            });
        }
...
يرجى فتح البريد الإلكتروني في محرك يدعم قراءة رسائل HTML لتتمكن من مراجعة الإجراءات السريعة وتفاصيل الحضور والغياب.
        ...
        if (action === "savePushSubscription") {
            ...
            if (!p256dh || !auth) {
                return res.status(200).json({ success: false, message: "مفاتيح الاشتراك غير مكتملة" });
            }
            ...
            if (findError) {
                return res.status(200).json({ success: false, message: "فشل التحقق من الاشتراك السابق" });
            }
            ...
            if (existing) {
                ...
                if (updateError) {
                    return res.status(200).json({ success: false, message: "فشل تحديث الاشتراك" });
                }
            } else {
                const { error: insertError } = await supabase
                    .from('push_subscriptions')
                ...
                if (insertError) {
                    return res.status(200).json({ success: false, message: "فشل حفظ الاشتراك الجديد" });
                }
            }

            return res.status(200).json({ success: true, message: "تم حفظ الاشتراك بنجاح" });
        }

        // --- GET TELEGRAM BOT LINK ---
        if (action === "getTelegramBotLink") {
            const { employeeId } = data;
            if (!employeeId) {
                return res.status(200).json({ success: false, message: "معرف الموظف مطلوب" });
            }

            const botUsername = process.env.TELEGRAM_BOT_USERNAME || 'MyCompanyBot';
            const token = crypto.randomBytes(8).toString('hex'); // 16 characters
            ...
            if (insertError) {
                return res.status(200).json({ success: false, message: "فشل إنشاء رمز الربط" });
            }
        ...
        if (action === "unlinkTelegram") {
            ...
            if (deleteError) {
                return res.status(200).json({ success: false, message: "فشل إلغاء ربط الحساب" });
            }

            return res.status(200).json({ success: true, message: "تم إلغاء ربط حساب تيليجرام بنجاح" });
        }
        ...
        if (action === "updateNotificationPreference") {
            ...
            if (updateError) {
                return res.status(200).json({ success: false, message: "فشل تحديث تفضيلات الإشعارات" });
            }

            return res.status(200).json({ success: true, message: "تم تحديث تفضيلات الإشعارات بنجاح" });
        }

        // --- GET EMPLOYEE NOTIFICATION STATE ---
        if (action === "getEmployeeNotificationState") {
            const { employeeId } = data;
            if (!employeeId) {
                return res.status(200).json({ success: false, message: "معرف الموظف مطلوب" });
            }
            ...
            if (empErr) {
                return res.status(200).json({ success: false, message: "فشل استرجاع بيانات الموظف" });
            }
            ...
            if (tgErr) {
                return res.status(200).json({ success: false, message: "فشل استرجاع ربط تيليجرام" });
            }

            return res.status(200).json({
                success: true,
                preferredChannel: employee?.preferred_notification_channel || 'both',
                isTelegramLinked: !!tgConn
            });
        }

        // --- TEST EMPLOYEE NOTIFICATIONS ---
        if (action === "testEmployeeNotifications") {
            const { employeeId } = data;
            if (!employeeId) {
                return res.status(200).json({ success: false, message: "معرف الموظف مطلوب" });
            }
            ...
            const results = { push: 'skipped', telegram: 'skipped', email: 'skipped' };
            const testPayload = {
                title: '🧪 اختبار نظام التنبيهات',
                body: 'مرحباً! هذا إشعار تجريبي لاختبار إعدادات التنبيهات الخاصة بك.',
            ...
            const preferredChannel = employee.preferred_notification_channel || 'both';

            // 1. Test Web Push (if push or both is preferred)
            if (preferredChannel === 'push' || preferredChannel === 'both') {
                ...
                if (pushSubs && pushSubs.length > 0) {
                    try {
                        ...
                        webpush.setVapidDetails('mailto:support@democompany.com', VAPID_PUBLIC_KEY, VAPID_PRIVATE_KEY);
                        ...
                        results.push = pushSuccess ? 'success' : 'failed';
                    } catch (err) {
                        console.error('Import web-push error:', err.message);
                        results.push = 'failed';
                    }
                } else {
            ...
            // 2. Test Telegram (if telegram or both is preferred)
            if (preferredChannel === 'telegram' || preferredChannel === 'both') {
                ...
                if (tgConn && process.env.TELEGRAM_BOT_TOKEN) {
                    try {
                        const tgRes = await fetch(`https://api.telegram.org/bot${process.env.TELEGRAM_BOT_TOKEN}/sendMessage`, {
                            ...
                            body: JSON.stringify({
                                chat_id: tgConn.telegram_chat_id,
                                text: '🧪 اختبار نظام التنبيهات:\nمرحباً! هذا إشعار تجريبي لاختبار قناة تيليجرام.'
                            })
                        });
                        const tgJson = await tgRes.json();
                        results.telegram = tgJson.ok ? 'success' : 'failed';
                    } catch (err) {
                        console.error('Test telegram error:', err.message);
                        results.telegram = 'failed';
                    }
                } else {
                    results.telegram = tgConn ? 'no_bot_token' : 'not_linked';
                }
            }

            // 3. Test Email (if email is preferred or if active channels failed/skipped)
            const shouldSendEmail = preferredChannel === 'email' || data.testEmail ||
                ((preferredChannel === 'push' || preferredChannel === 'both') && results.push !== 'success') ||
                ((preferredChannel === 'telegram' || preferredChannel === 'both') && results.telegram !== 'success');
            ...
            if (shouldSendEmail && employee.email) {
                try {
                    const emailRes = await sendEmailNotification({
                        ...
                        text: 'مرحباً! هذا إشعار تجريبي لاختبار قناة البريد الإلكتروني.',
                        html: '<p>مرحباً! هذا إشعار تجريبي لاختبار قناة البريد الإلكتروني.</p>'
                    });
                    results.email = emailRes.success ? 'success' : 'failed';
                } catch (err) {
            ...
            // Log successful notification sends
            const loggedChannels = [];
            if (results.push === 'success') loggedChannels.push('push');
            if (results.telegram === 'success') loggedChannels.push('telegram');
            if (results.email === 'success') loggedChannels.push('email');

            for (const ch of loggedChannels) {
                try {
                    await supabase
                        ...
                        .insert([{
                            ...
                            slot: 'test',
                            status: 'success',
                            notification_date: getCairoDateString(new Date()),
                            sent_at: new Date().toISOString()
                        }]);
                } catch (err) {
                    console.error('Log test notification error:', err.message);
                }
            }

            return res.status(200).json({
                success: true,
                message: "تم إجراء الفحص والاختبار بنجاح",
                results: results
            });
        }
