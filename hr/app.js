...
async function editEmployee(id) {
    ...
    document.getElementById('empMonthlySalary').value = emp.monthly_salary || 0;
...
async function saveEmployee() {
    ...
    const monthlySalary = document.getElementById('empMonthlySalary').value || 0;
    const transportPrice = document.getElementById('empTransportPrice').value || 0;
    ...
    const payload = {
        ...
        transportPrice: parseFloat(transportPrice || 0),
...
// ------ ALLOWANCE REQUESTS ------ //
async function fetchAllowanceRequests() {
    document.getElementById('loader').classList.remove('hidden');
    try {
        const res = await fetch(`${API_URL}?action=getAllowanceRequests`);
        const result = await res.json();
        if (result.success) {
            renderAllowanceRequests(result.data);
        }
    } catch (e) {
        console.error(e);
    }
...
function renderAllowanceRequests(data) {
    const tbody = document.getElementById('allowanceRequestsTableBody');
    tbody.innerHTML = '';
    
    [...data].forEach(req => {
        const dateStr = new Date(req.date).toLocaleDateString('ar-EG');
        const statusMap = {
            'pending': { text: 'قيد الانتظار', color: 'orange' },
            'approved': { text: 'تمت الموافقة', color: 'var(--secondary)' },
            'rejected': { text: 'مرفوض', color: 'var(--danger)' }
        };
        const statusMeta = statusMap[req.status] || { text: req.status, color: '#fff' };
        
        const mapUrl = `https://www.google.com/maps?q=${req.latitude},${req.longitude}`;
        
        tbody.innerHTML += `
            <tr>
                <td>${req.employees?.name || req.employeeId}</td>
                <td>${dateStr}</td>
                <td>${req.extraAmount} ج.م</td>
                <td>${req.note || '-'}</td>
                <td><a href="${mapUrl}" target="_blank">📍 عرض الموقع</a></td>
                <td><span style="color:${statusMeta.color}">${statusMeta.text}</span></td>
                <td>
                    ${req.status === 'pending' ? `
                        <button class="btn-primary" style="background:var(--secondary); width:auto; padding:5px 10px;" onclick="updateAllowanceStatus('${req.id}', 'approved')">موافقة</button>
                        <button class="btn-danger" style="width:auto; padding:5px 10px;" onclick="updateAllowanceStatus('${req.id}', 'rejected')">رفض</button>
                    ` : '-'}
                </td>
            </tr>
        `;
...
async function updateAllowanceStatus(id, status) {
    ...
    try {
        const res = await fetch(API_URL, {
            method: 'POST',
            body: JSON.stringify({ action: 'updateAllowanceStatus', id, status }),
            headers: { 'Content-Type': 'text/plain' }
        });
        const result = await res.json();
        if (result.success) {
    ...
    data.forEach(hol => {
        tbody.innerHTML += `
            <tr>
                <td>${hol.date}</td>
                <td>${hol.name}</td>
                <td>${hol.type === 'official' ? 'رسمية' : 'عطلة'}</td>
                <td>
                    <button class="btn-danger" style="width:auto; padding:5px 10px;" onclick="deleteHoliday('${hol.id}')">حذف</button>
                </td>
            </tr>
        `;
    });
...
async function saveHoliday() {
    ...
    if (!date || !name) return alert("يرجى إكمال البيانات");
...
async function deleteHoliday(id) {
    ...
    try {
        const res = await fetch(API_URL, {
            method: 'POST',
            body: JSON.stringify({ action: 'deleteHoliday', id }),
            headers: { 'Content-Type': 'text/plain' }
        });
        const result = await res.json();
        if (result.success) {
