...
// Supabase Configuration - Loaded from environment via API
let supabase = null;
...
function showLiveNotification(message, type = 'info') {
    ...
    notification.innerHTML = `
        <div style="display: flex; align-items: center; gap: 8px;">
            <span style="font-size: 18px;">${type === 'success' ? '✅' : type === 'warning' ? '⚠️' : 'ℹ️'}</span>
            <span>${message}</span>
        </div>
    `;
...
async function initSupabaseRealtime() {
    ...
    try {
        ...
        const res = await fetch(`${API_URL}?action=getSupabaseConfig`);
        const config = await res.json();

        if (!config.success) {
            console.warn('Realtime not available:', config.message);
            return;
        }
        ...
        subscribeToSiteRequests();
        subscribeToAllowanceRequests();
        ...
        console.warn('Failed to init realtime:', e);
    }
}
...
function handleAttendanceChange(payload) {
    ...
    if (eventType === 'INSERT') {
        ...
        // Show notification
        const empName = newRecord.employeeName || 'موظف';
        const action = newRecord.checkOut ? '🔴 Check-out' : '🟢 Check-in';
        showLiveNotification(`${action}: ${empName} في ${newRecord.siteName || 'الموقع'}`, 'success');
        
        // Update UI if on attendance tab
        ...
        // Update local data
        const index = allAttendanceData.findIndex(a => a.id === newRecord.id);
        if (index !== -1) {
            const hadCheckOut = allAttendanceData[index].checkOut;
            allAttendanceData[index] = newRecord;
            
            // Notify if checkout was added
            if (!hadCheckOut && newRecord.checkOut) {
                showLiveNotification(`🔴 Check-out: ${newRecord.employeeName || 'موظف'}`, 'info');
            }
        }
...
function subscribeToSiteRequests() {
    const channel = supabase
        .channel('site-requests-changes')
        .on('postgres_changes',
            { event: '*', schema: 'public', table: 'siteRequests' },
            (payload) => handleSiteRequestChange(payload)
        )
        .subscribe();
    realtimeChannels.push(channel);
}

function handleSiteRequestChange(payload) {
    const { eventType, new: newRecord } = payload;
    const activeTab = localStorage.getItem('hrActiveTab');

    if (eventType === 'INSERT') {
        allSiteRequests.unshift(newRecord);
        showLiveNotification(`📍 طلب موقع جديد من: ${newRecord.employeeName || 'موظف'}`, 'warning');
        
        if (activeTab === 'siteRequests') {
            renderRequestsTable(allSiteRequests);
        }
        updateDashboardStats();
        
    } else if (eventType === 'UPDATE') {
        const index = allSiteRequests.findIndex(r => r.id === newRecord.id);
        if (index !== -1) {
            allSiteRequests[index] = newRecord;
        }
...
function subscribeToAllowanceRequests() {
    const channel = supabase
        .channel('allowance-changes')
        .on('postgres_changes',
            { event: '*', schema: 'public', table: 'allowanceRequests' },
            (payload) => handleAllowanceChange(payload)
        )
        .subscribe();
    realtimeChannels.push(channel);
}

function handleAllowanceChange(payload) {
    const { eventType, new: newRecord } = payload;
    const activeTab = localStorage.getItem('hrActiveTab');

    if (eventType === 'INSERT') {
        allAllowanceRequests.unshift(newRecord);
        showLiveNotification(`💰 طلب بدل جديد من: ${newRecord.employeeName || 'موظف'}`, 'warning');
        
        if (activeTab === 'allowanceRequests') {
            renderAllowanceRequestsTable(allAllowanceRequests);
        }
        updateDashboardStats();
        
    } else if (eventType === 'UPDATE') {
        const index = allAllowanceRequests.findIndex(r => r.id === newRecord.id);
        if (index !== -1) {
            allAllowanceRequests[index] = newRecord;
        }
...
function subscribeToEmployees() {
    const channel = supabase
        ...
        .on('postgres_changes',
            ...
            (payload) => handleEmployeeChange(payload)
        )
        .subscribe();
    realtimeChannels.push(channel);
}
...
function handleEmployeeChange(payload) {
    ...
    if (eventType === 'INSERT') {
        ...
        showLiveNotification(`👤 موظف جديد مسجل: ${newRecord.name}`, 'success');
        ...
        const index = allEmployees.findIndex(e => e.id === newRecord.id);
        if (index !== -1) {
            allEmployees[index] = newRecord;
        }
        ...
        allEmployees = allEmployees.filter(e => e.id !== oldRecord.id);
        showLiveNotification(`🗑️ تم حذف موظف`, 'info');
...
function updateDashboardStats() {
    ...
    const todayAttendance = allAttendanceData.filter(a => 
        a.checkIn && a.checkIn.startsWith(today)
    );
    
    const activeNow = todayAttendance.filter(a => a.checkIn && !a.checkOut).length;
    const pendingRequests = allSiteRequests.filter(r => r.status === 'pending').length;
    const pendingAllowances = allAllowanceRequests.filter(r => r.status === 'pending').length;
    ...
    updateElement('statActiveNow', activeNow);
    updateElement('statPendingRequests', pendingRequests);
    updateElement('statPendingAllowances', pendingAllowances);
    updateElement('statTotalEmployees', allEmployees.length);
}
